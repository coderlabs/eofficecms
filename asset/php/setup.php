...
<?php require_once("admin_files/header.php"); ?>
		...
		1 => array ("desc" => "exec() enabled",
				    ...
				    "sev" => 0),
		2 => array ("desc" => "Max upload size > 10 mb",
		...
		if($configManager->getConfig('test_pdf2json')){
			$test = array ("desc" => "PDF2JSON support",
					    "test" => $pdf2json,
					...
					function checkDirectoryPermissions(obj){
						...
						$.ajax({
						  ...
						  success: function(data){
						  	if(data=="0"){
								$('#'+infield.attr("id")+'_ERROR').html('Cannot write to directory. Please verify path & permissions (needs to be writable).<button class="tiny main n_button" type="submit" onclick="checkDirectoryPermissions(jQuery(\'input#'+infield.attr('id')+'\'));return false;"><span></span><em style="min-width:100px">retry <img src="admin_files/images/reload.png"  style="margin-top:3px"/></em></button>');
								return false;
							}else{
